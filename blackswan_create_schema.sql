-- MySQL Script generated by MySQL Workbench
-- Wed 17 Mar 2021 11:12:08 AM CDT
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- -----------------------------------------------------
-- Schema blackswan_event_tracker
-- -----------------------------------------------------
DROP SCHEMA IF EXISTS `blackswan_event_tracker` ;

-- -----------------------------------------------------
-- Schema blackswan_event_tracker
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `blackswan_event_tracker` ;
USE `blackswan_event_tracker` ;

-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`user`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`user` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`user` (
  `idUser` INT NOT NULL,
  `username` VARCHAR(90) NOT NULL,
  `website` VARCHAR(90) NOT NULL,
  `displayname` VARCHAR(90) NULL,
  `follower_count` INT NULL,
  `follow_count` INT NULL,
  PRIMARY KEY (`idUser`),
  UNIQUE INDEX `username_UNIQUE` (`username` ASC) ,
  UNIQUE INDEX `website_UNIQUE` (`website` ASC) )
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`post`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`post` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`post` (
  `idPost` INT NOT NULL,
  `title` VARCHAR(90) NULL,
  `date` DATE NULL,
  `time` TIME NULL,
  `description` VARCHAR(500) NULL,
  `like_num` INT NULL,
  `comment_num` INT NULL,
  `dislike_num` INT NULL,
  `is_comment` TINYINT NULL,
  `parentid` INT NULL,
  `url` VARCHAR(45) NULL,
  `insensitive` TINYINT NULL,
  `language` VARCHAR(45) NULL,
  `sharecount` INT NULL,
  `idUser` INT NULL,
  PRIMARY KEY (`idPost`),
  INDEX `idUser_idx` (`idUser` ASC) ,
  CONSTRAINT `idUser`
    FOREIGN KEY (`idUser`)
    REFERENCES `blackswan_event_tracker`.`user` (`idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`location`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`location` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`location` (
  `idLocation` INT NOT NULL,
  `gps_long` DOUBLE NULL,
  `gps_lat` DOUBLE NULL,
  `name` VARCHAR(90) NULL,
  `radius` DOUBLE NULL,
  PRIMARY KEY (`idLocation`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`event`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`event` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`event` (
  `idevent` INT NOT NULL,
  `name` VARCHAR(45) NOT NULL,
  `date_start` DATE NULL,
  `time_start` TIME NULL,
  `date_end` DATE NULL,
  `time_end` TIME NULL,
  PRIMARY KEY (`idevent`),
  UNIQUE INDEX `name_UNIQUE` (`name` ASC) )
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`event_location`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`event_location` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`event_location` (
  `idLocation` INT NOT NULL,
  `idevent` INT NOT NULL,
  INDEX `idevent_idx` (`idevent` ASC) ,
  CONSTRAINT `idLocation`
    FOREIGN KEY (`idLocation`)
    REFERENCES `blackswan_event_tracker`.`location` (`idLocation`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idevent`
    FOREIGN KEY (`idevent`)
    REFERENCES `blackswan_event_tracker`.`event` (`idevent`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`user_relationship`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`user_relationship` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`user_relationship` (
  `idUser` INT NOT NULL,
  `idUser2` INT NOT NULL,
  CONSTRAINT `idUser`
    FOREIGN KEY (`idUser` , `idUser2`)
    REFERENCES `blackswan_event_tracker`.`user` (`idUser` , `idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`tag`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`tag` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`tag` (
  `idtag` INT NOT NULL,
  `name` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idtag`),
  UNIQUE INDEX `name_UNIQUE` (`name` ASC) )
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`tag_multirelation`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`tag_multirelation` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`tag_multirelation` (
  `idtag` INT NOT NULL,
  `idUser` INT NULL,
  `idLocation` INT NULL,
  `idevent` INT NULL,
  `idPost` INT NULL,
  INDEX `idUser_idx` (`idUser` ASC) ,
  INDEX `idLocation_idx` (`idLocation` ASC) ,
  INDEX `idevent_idx` (`idevent` ASC) ,
  INDEX `idPost_idx` (`idPost` ASC) ,
  CONSTRAINT `idtag`
    FOREIGN KEY (`idtag`)
    REFERENCES `blackswan_event_tracker`.`tag` (`idtag`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idUser`
    FOREIGN KEY (`idUser`)
    REFERENCES `blackswan_event_tracker`.`user` (`idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idLocation`
    FOREIGN KEY (`idLocation`)
    REFERENCES `blackswan_event_tracker`.`location` (`idLocation`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idevent`
    FOREIGN KEY (`idevent`)
    REFERENCES `blackswan_event_tracker`.`event` (`idevent`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idPost`
    FOREIGN KEY (`idPost`)
    REFERENCES `blackswan_event_tracker`.`post` (`idPost`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`url`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`url` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`url` (
  `idurl` INT NOT NULL,
  `url` VARCHAR(500) NOT NULL,
  PRIMARY KEY (`idurl`),
  UNIQUE INDEX `url_UNIQUE` (`url` ASC) )
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`url_post`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`url_post` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`url_post` (
  `idPost` INT NOT NULL,
  `idurl` INT NOT NULL,
  INDEX `idurl_idx` (`idurl` ASC) ,
  CONSTRAINT `idPost`
    FOREIGN KEY (`idPost`)
    REFERENCES `blackswan_event_tracker`.`post` (`idPost`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idurl`
    FOREIGN KEY (`idurl`)
    REFERENCES `blackswan_event_tracker`.`url` (`idurl`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`locationpost`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`locationpost` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`locationpost` (
  `idLocation` INT NOT NULL,
  `idPost` INT NOT NULL,
  INDEX `idPost_idx` (`idPost` ASC) ,
  CONSTRAINT `idPost`
    FOREIGN KEY (`idPost`)
    REFERENCES `blackswan_event_tracker`.`post` (`idPost`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idLocation`
    FOREIGN KEY (`idLocation`)
    REFERENCES `blackswan_event_tracker`.`location` (`idLocation`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`media`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`media` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`media` (
  `idmedia` INT NOT NULL,
  `data` BLOB NOT NULL,
  `media_type` VARCHAR(45) NULL,
  `runtime` TIME NULL,
  PRIMARY KEY (`idmedia`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `blackswan_event_tracker`.`media_post`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `blackswan_event_tracker`.`media_post` ;

CREATE TABLE IF NOT EXISTS `blackswan_event_tracker`.`media_post` (
  `idPost` INT NOT NULL,
  `idmedia` INT NOT NULL,
  INDEX `idPost_idx` (`idPost` ASC) ,
  INDEX `idmedia_idx` (`idmedia` ASC) ,
  CONSTRAINT `idPost`
    FOREIGN KEY (`idPost`)
    REFERENCES `blackswan_event_tracker`.`post` (`idPost`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idmedia`
    FOREIGN KEY (`idmedia`)
    REFERENCES `blackswan_event_tracker`.`media` (`idmedia`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;

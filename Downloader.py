...
def donwloadMedia(tweetID):
    ...
    if tweetHasMedia(tweet) == False:
        ...
        if mediaType(tweet) == "video":
            ...
            for tweet.extended_entities.media.media_url in tweet.extended_entities.media:
                wget.download(tweet.extended_entities.media.media_url)
...
def mediaType(tweet):
    if tweet.extended_entities.media.type == "photo":
        return "photo"
    elif tweet.extended_entities.media.type == "video":
        return "video"
    else:
        return "animated_gif"
